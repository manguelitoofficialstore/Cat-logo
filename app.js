 ...
 * Categorías: Ropa → Ítems → Accesorios.
 * Dentro de Ropa: Suéteres → Blusas.
 * Cualquier categoría o subcategoría nueva que no aparezca aquí
 * se mostrará después, en orden alfabético.
 */
const CATEGORY_PRIORITY_GROUPS = Object.freeze([
  Object.freeze(["ropa"]),
  Object.freeze(["item", "items"]),
  Object.freeze(["accesorio", "accesorios"])
]);

const ROPA_SUBCATEGORY_PRIORITY_GROUPS = Object.freeze([
  Object.freeze(["sueter", "sueteres", "sweater", "sweaters"]),
  Object.freeze(["blusa", "blusas"])
]);

function getPriorityIndex(value, priorityGroups) {
...
function compareCategories(categoryA, categoryB) {
  const priorityA = getPriorityIndex(categoryA, CATEGORY_PRIORITY_GROUPS);
  const priorityB = getPriorityIndex(categoryB, CATEGORY_PRIORITY_GROUPS);
...
function compareSubcategories(subcategoryA, subcategoryB, category) {
  const normalizedCategory = normalizeText(category).trim();

  if (normalizedCategory === "ropa") {
    const priorityA = getPriorityIndex(
      subcategoryA,
      ROPA_SUBCATEGORY_PRIORITY_GROUPS
    );
    const priorityB = getPriorityIndex(
      subcategoryB,
      ROPA_SUBCATEGORY_PRIORITY_GROUPS
    );
  ...
  return subcategoryA.localeCompare(subcategoryB, "es", {
    sensitivity: "base"
  });
}
...
function renderCategoryButtons() {
  ...
  [...hierarchy.entries()]
    ...
    .forEach(([category, subcategories]) => {
      ...
      [...subcategories]
        .sort((a, b) => compareSubcategories(a, b, category))
        .forEach((subcategory) => {
...
function setProductImage(img, fileName, folder) {
  ...
  // Evita que una imagen anterior permanezca visible mientras se prueba la nueva.
  img.onerror = null;
  img.removeAttribute("src");

  if (!raw) {
    img.src = fallback;
    img.dataset.imageStatus = "sin-nombre";
    return;
  }

  // El Excel debe contener el nombre del archivo, con o sin .png.
  // Si por error contiene una ruta, conservamos solo el nombre final.
  ...
  if (!originalName) {
    img.src = fallback;
    img.dataset.imageStatus = "nombre-invalido";
    return;
  }

  const extensionMatch = originalName.match(/\.([a-z0-9]+)$/i);
  const baseName = extensionMatch
    ? originalName.slice(0, -extensionMatch[0].length)
    : originalName;

  // Se prueban únicamente formatos de imagen web; nunca archivos SVG provisionales.
  // La primera opción siempre es el nombre exacto escrito en el Excel.
  const names = [];
  const addName = (value) => {
  ...
  if (extensionMatch) {
    addName(originalName);
    addName(`${baseName}.png`);
    addName(`${baseName}.PNG`);
  } else {
    addName(`${originalName}.png`);
    addName(`${originalName}.PNG`);
  }

  // Variantes defensivas para los errores más comunes al subir a GitHub.
  // GitHub Pages distingue mayúsculas, minúsculas, espacios, guiones y guiones bajos.
  const baseVariants = [
    baseName,
  ...
  const requestedFolders = folder === "ampliadas"
    ? ["ampliadas", "catalogo"]
    : ["catalogo", "ampliadas"];
  ...
  requestedFolders.forEach((candidateFolder) => {
    names.forEach((name) => {
      addPath(`assets/productos/${candidateFolder}/${encodeURIComponent(name)}`);
    });
  });

  // También se prueba la carpeta productos por si la imagen fue subida un nivel arriba.
  names.forEach((name) => {
    addPath(`assets/productos/${encodeURIComponent(name)}`);
  });

  const cacheVersion = window.MOSS_IMAGE_VERSION || "20260710-4";
  let candidateIndex = 0;

  const tryCandidate = () => {
    if (candidateIndex >= candidatePaths.length) {
      console.error(
        `No se encontró la imagen "${originalName}". Rutas probadas:`,
        candidatePaths
      );
      img.onerror = null;
      img.src = `${fallback}?v=${cacheVersion}`;
      img.dataset.imageStatus = "no-encontrada";
      img.title = `Imagen no encontrada: ${originalName}.png`;
      return;
    }

    const candidate = candidatePaths[candidateIndex];
    candidateIndex += 1;
    img.dataset.requestedImage = candidate;
    img.dataset.imageStatus = "probando";
    img.src = `${candidate}?v=${cacheVersion}`;
  };

  img.onload = () => {
    img.dataset.imageStatus = img.src.includes(fallback) ? "respaldo" : "cargada";
    if (!img.src.includes(fallback)) img.removeAttribute("title");
  };
