...
function setProductImage(img, fileName, folder) {
  ...
  // Las imágenes del catálogo se manejan como PNG.
  // Si en el Excel no se escribe la extensión, se agrega .png automáticamente.
  // Ya no se intenta cargar ningún archivo SVG provisional.
  const hasExtension = /\.[a-z0-9]+$/i.test(safeName);
  const pngName = hasExtension
    ? safeName.replace(/\.[^.]+$/, ".png")
    : `${safeName}.png`;
  const encodedName = pngName
    .split("/")
    .map((part) => encodeURIComponent(part))
    .join("/");

  const folders = folder === "ampliadas"
    ? ["ampliadas", "catalogo"]
    : ["catalogo"];

  const candidates = folders.map(
    (candidateFolder) => `assets/productos/${candidateFolder}/${encodedName}`
  );
